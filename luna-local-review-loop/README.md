...
Before native selection, the parent validates the canonical repository and registry authority, then checks the project-local fallback registry, when present, for an active or awaiting-continue task with the same immutable scope. A missing registry is the explicit `not-initialized` clear state; malformed, unreadable, or ambiguous fallback state fails closed. An active task remains the owner until retired; native cannot start over it. Immediately after that preflight and before native spawn, the parent invokes `bash scripts/cross-path-claim.sh acquire --repo "$repo_root" --scope "$scope" --token "$native_task_id"`, using the stable unique native task identity as token. This atomic host-owned claim is keyed by the physical checkout identity and scope. The claim is distinct from the fallback registry: native neither reads nor writes registry state. For non-terminal continuation, native explicitly invokes the same command with `--reenter` and the same stable token. After cleanup and terminal retirement, native invokes `bash scripts/cross-path-claim.sh release --repo "$repo_root" --scope "$scope" --token "$native_task_id"`; release never guesses ownership. The fallback reservation invokes the same primitive before its registry reservation, and both paths hold it through their complete lifecycle and cleanup. An unavailable or conflicting claim is a parent-action stop; neither path starts. This claim closes the check-then-start race in which a fallback controller could reserve the same scope after native preflight but before native startup.

Fallback launch runs `registry.sh preflight` inside cross-path claim setup: the claim key is derived without a checkout seal, its per-claim lock is acquired, existing project-local boundary, private registry-local ignore, effective root ignore, and schema-v1/v2 or pre-seal schema-v3 migration/recovery eligibility are validated, then the seal is validated or created and the claim is published. A failed preflight releases coordination without publishing a claim or creating a seal; registry and ignore bytes remain unchanged. Native acquisition never reads or writes fallback registry state, and claim-before-reservation serialization remains required.
...
Run this section only after the native capability gate reports that native startup is unavailable, fallback ownership has been pinned or proven clear, the cross-path claim is acquired for the canonical checkout and immutable scope, and the CLI Sol-High reviewer route is available or explicitly Evidence blocked.
...
The fallback controller acquires the host-owned cross-path claim before `registry.sh reserve`, holds it through the CLI worker and cleanup lifecycle, and releases it only after the registry task is terminal. A non-terminal `needs_parent_action` checkpoint retains the claim, and the same task identity re-enters it for continuation. A legacy active project-local task created before cross-path claims may continue only after its exact active registry row is validated, using `--reenter-or-acquire`: a missing claim is acquired atomically, while an existing claim still requires exact checkout identity, scope, and token ownership. Until a legacy controller proves registry-invocation ownership, failure cleanup retains even a newly acquired claim; completed or blocked retirement releases it, while `needs_parent_action` retains it for re-entry. A claim held by native, unavailable, malformed, or conflicting cannot be bypassed by reserving or continuing the fallback registry.
