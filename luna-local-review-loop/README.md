...
Immediately after that preflight and before native spawn, the parent invokes `bash "$skill_root/scripts/cross-path-claim.sh" acquire --repo "$repo_root" --scope "$scope" --token "$native_task_id" --pid "$$"`, using the stable unique native task identity as token and the long-lived native controller PID as owner identity. This atomic host-owned claim is keyed by the physical checkout identity and scope. The claim is distinct from the fallback registry: native neither reads nor writes registry state. For non-terminal continuation, native explicitly invokes `bash "$skill_root/scripts/cross-path-claim.sh" acquire --reenter --repo "$repo_root" --scope "$scope" --token "$native_task_id" --pid "$$"`. After cleanup and terminal retirement, native invokes `bash "$skill_root/scripts/cross-path-claim.sh" release --repo "$repo_root" --scope "$scope" --token "$native_task_id" --pid "$$"`; the same live controller PID must flow through acquire, re-entry, and release so the claim can validate its process instance. The fallback launcher invokes `run-worker.sh launch`, which uses `--reenter-or-acquire` with stable token `task-$TASK_ID` before its registry reservation: a parent-held same-owner claim is explicitly re-entered, while a missing claim is acquired atomically. Both paths hold their claim through lifecycle and cleanup. An unavailable or competing claim is a parent-action stop; neither path starts. This claim closes the check-then-start race in which a fallback controller could reserve the same scope after native preflight but before native startup.
