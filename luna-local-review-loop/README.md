...
Init is idempotent and does not edit tracked project files. It physically resolves existing path components before normalizing missing suffixes, rejects repository-local state before creation, and rejects a symlinked repository-fingerprint directory before chmod or write. On first initialization it creates one random, untracked `luna-local-review-loop.instance` marker inside the physical Git directory, then binds external state to that non-reusable identity. Later lookups scan the state root for that identity, so moving the same checkout reuses its registry and updates the stored canonical root even though the directory retains its original creation checksum. If a different repository later occupies the old path, its missing or different marker prevents attachment to those sessions. When required project skills are missing, init prints the explicit universal-target install commands. Run those only as a separate, intentional project change.
...
Only that JSON result is emitted on stdout. The external artifact directory printed on stderr contains the handshake JSONL, each resume JSONL, separate stderr logs, and each final result. Artifact roots, task directories, individual logs, results, and tracker state are rejected when symlinked or resolved to an unsafe target. A registry-backed invocation claim serializes the first resume, later continuations, and explicit retirement; continuation claims atomically require active state, and the initial claim is stored in the same mutation as reservation. Every Codex launch waits behind a start gate in its own process group until that PGID is durable. An ancestry tracker also records live descendants that create new process groups. Stale claims are reclaimed only after both the original group and the tracked process tree are confirmed empty; zombies count as exited, while ambiguous access blocks recovery rather than risking unsafe retirement. The mutation lock writes its owner PID before atomically publishing a regular lock file by hard link. Release and stale recovery compare the retained inode, ownerless stale files are recoverable, and symlinked/non-regular locks are rejected. Runner termination drains the original Codex group and every tracked descendant before token-checked retirement. This prevents concurrent or orphaned registry transitions, detached repository-writing descendants, and session resumes while keeping long streams, repeated diffs, reconnect noise, or unrelated warnings from burying or corrupting the final report.
