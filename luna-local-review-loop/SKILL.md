...
Init validates `codex`, `jq`, Git, project-local Caveman, and project-local code-reviewer. It stores the registry below `${LUNA_REGISTRY_ROOT:-${TMPDIR:-/tmp}/luna-local-review-loop}` using a deterministic creation key. First initialization creates one random, untracked `luna-local-review-loop.instance` marker inside the physical Git directory and binds external state to that non-reusable repository identity. Later lookups resolve that identity across the state root, so moving the same checkout preserves live workers and updates the stored canonical repository root; a replacement repository at the old path cannot inherit them. A live project-local schema-v1 registry blocks only first-time external-state creation; once valid external state exists, recovery access remains available even if legacy state reappears. Init never edits `.gitignore`, `skills-lock.json`, `.agents/skills`, or tracked project files, and never installs from the network. If a skill is missing, init prints explicit universal-target install commands; the parent decides whether to run them and reviews the resulting project changes.
...
The registry atomically claims each live invocation. A stale claim may be replaced only while holding the registry mutation lock. The lock owner writes its PID before atomically publishing the regular lock file by hard link; release and stale recovery compare the retained inode, so a contender cannot delete a newer owner, and symlinked/non-regular locks are rejected. Every Codex launch runs in its own process group and waits behind a start gate until its PGID is durably recorded. A separate ancestry tracker records live descendants even when they create new process groups, and recovery refuses child clearing or retirement until both the original group and the tracked process tree are confirmed empty, including after a runner crash or `SIGKILL`. Zombie members count as exited. Ambiguous access is handled conservatively as still live, requiring manual inspection instead of unsafe retirement. If the runner receives `INT` or `TERM`, it signals and drains the Codex group plus tracked descendants before token-checked registry retirement. Artifact directories and files must remain real and non-symlinked below the external registry path. Never bypass these controls by deleting registry state manually.
